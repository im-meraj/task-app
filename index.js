...
const taskModal = document.querySelector(".task__body");
...
const saveToLocalStorage = () =>
  ...
  localStorage.setItem("taskyMERAJ", JSON.stringify({ card: globalTaskData }));
...
const loadExistingCards = () => {
  //check local storage
  const getData = localStorage.getItem("taskyMERAJ");
  //parse data, if exist
  if (!getData) return;
...
const searchTask = (e) => {
  ...
  const results = globalTaskData.filter((task) => {
    return (
